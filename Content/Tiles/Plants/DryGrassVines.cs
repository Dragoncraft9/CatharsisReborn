...
{
    public class DryGrassVines : ModTile
    {
    ...
    {
        private int DryGrassVines;
        private int DryGrass;

        public override void SetStaticDefaults()
        {
            // Caching these tile type values to make the code more readable
            DryGrassVines = ModContent.TileType<DryGrassVines>();
            DryGrass = ModContent.TileType<DryGrass>();
        }
        ...
        public override void RandomUpdate(int i, int j, int type)
        {
            ...
            if ((tile.TileType == DryGrassVines || tile.TileType == DryGrass) && WorldGen.GrowMoreVines(i, j))
            {
                int growChance = 70;
                if (tile.TileType == DryGrassVines)
                {
                ...
                if (WorldGen.genRand.NextBool(growChance) && !tileBelow.HasTile && tileBelow.LiquidType != LiquidID.Lava)
                {
                    ...
                    for (int above = j; above > j - 10; above--)
                    {
                        ...
                        if (tileAbove.HasTile && tileAbove.TileType == DryGrass && !tileAbove.BottomSlope)
                        {
                    ...
                    if (vineIsHangingOffValidTile)
                    {
                        ...
                        tileBelow.TileType = (ushort)DryGrassVines;
        ...
        public override bool TileFrame(int i, int j, int type, ref bool resetFrame, ref bool noBreak)
        {
            ...
            if (type != aboveTileType)
            {
                ...
                if ((aboveTileType == DryGrass || aboveTileType == DryGrassVines) && type != DryGrassVines)
                {
                    tile.TileType = (ushort)DryGrassVines;
                ...
                if (type == DryGrassVines && aboveTileType != DryGrass)
                {
                    if (aboveTileType == -1)
                    {
